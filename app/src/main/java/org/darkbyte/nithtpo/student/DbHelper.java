...
public class DbHelper extends SQLiteOpenHelper {
    ...
    private static final String DB_NAME = "onesignalnotification";
    // HOME POSTS TABLE
    ...
    public List gethomedata() {
        ...
        List<notification_model> homedetails = new ArrayList<>();
        ...
        if (cursor != null) {
            try {
                if (cursor.moveToFirst()) {
                    do {
                        notification_model homeposts = new notification_model();
    ...
    public Cursor homeposteinnerdata(String id) {
        String query = "select * from " + TABLE_HOMEPOST + "  where  id = '" + id+"' order by id " ;
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery(query, null);
        return cursor;
    }
